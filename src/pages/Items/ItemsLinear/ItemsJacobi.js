...
import { Jacobi } from '../../../services/service' 
...
const ItemsJacobi = () => {
  ...
  const Calculate = () =>{
    ...
    A =[];
    Xi = [];
    // inputX=math.zeros(sizeInput)._data ;
    let result = [];
    if(sizeInput <= 8 && sizeInput > 0){
      ...
      if(sizeInput > 0){
        // result = Jacobi(A,B,sizeInput,sizeInput);
        result = Jacobi(A,B,Xi);
        console.log(result)
        if(!result){
          setPosts([]);
        }
      ...
      <BodyContent>
        ...
        <ItemFrom>
          <ContainerInput>
            ...
            <table>
              {
                currentPosts.length>0 && <Table style={{fontSize:'8px'}}colomn={currentPosts} loading={loading} Attribute={'Jabobi'} indexOfFirstPost={indexOfFirstPost} postsPerPage={postsPerPage} size={sizeInput}/>
              }
            </table>
