...
import { GaussSeidel } from '../../../services/service' 
...
const ItemsGaussSeidel = () => {
  ...
  const Calculate = () =>{
    ...
    if(sizeInput <= 8 && sizeInput > 0){
      ...
      if(sizeInput > 0){
        // result = GaussSeidel(A,B,inputX,sizeInput);
        result = GaussSeidel(A,B,Xi,sizeInput);
        if(!result){
          setPosts([]);
        }
        else{
  ...
  async function setExelample(){
    await axios.get("http://localhost:5000/api/example/GaussSeidel?token_api=la2UEiUaQD1XttfIX19Ub4t9qx1rcEF1YaenLS1bMZo=")
...
export default ItemsGaussSeidel;
