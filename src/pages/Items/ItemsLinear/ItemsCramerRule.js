...
import { CramerRule } from '../../../services/service' 
import {
  ItemFrom,
  BodyContent,
  ContainerInput,
...
const ItemsCramerRule = () => {
  ...
  const ChangeSizeInput = ()=>{
      ...
      if(B.length > 0){
        for(let j = 0 ; j < sizeInput ; j++){
          document.getElementById(`B:${j}`).value = "";
        }
      }
  }
  ...
  const Calculate = () =>{
    ...
    if(sizeInput <= 8 && sizeInput > 0){
      ...
      if(sizeInput > 0){
        result = CramerRule(parseInt(sizeInput),A,B);
        if(!result){
          setMatrixResult([])
        }
        else{
...
export default ItemsCramerRule;
